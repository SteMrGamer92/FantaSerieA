...
SUPABASE_URL = os.getenv('SUPABASE_KEY' , 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImlwcXhqdWRseGNxYWNndG1wa3p4Iiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc1OTEyNjU3OSwiZXhwIjoyMDc0NzAyNTc5fQ.9nMpSeM-p5PvnF3rwMeR_zzXXocyfzYV24vau3AcDso')
...
def fetch_page(url):
    ...
    OTTIMIZZATO per Render Free Tier (512MB RAM)
    """
    try:
        with sync_playwright() as p:
            print(f"  🌐 Avvio browser...")
            
            # ✅ OTTIMIZZAZIONI per server con poca memoria
            browser = p.chromium.launch(
                headless=True,
                args=[
                    '--disable-gpu',
                    '--disable-dev-shm-usage',  # Non usa /dev/shm (RAM limitata)
            ...
            # ATTENDI che gli elementi siano caricati
            ...
            # ✅ IMPORTANTE: Chiudi tutto per liberare memoria
            page.close()
            context.close()
            browser.close()
